 ...
 */
public class TwoJRTerminal {

    public TwoJRTerminal() {

    }

    private Scanner reader = new Scanner(System.in);
    private int port;
    private TwoJRRadioListener twoJrRadio;
    ...
    public void printHelpMenu() {
        ...
        //System.out.println("-ir: Initialize Radio");
        //System.out.println("-sd: Send data over radio");
        //System.out.println("-rd: Read data over radio");
    ...
    public void printDataTypes() {
        ...
        System.out.println("10: Application Packet");
    ...
    public void printNetworkControlTypes() {
        ...
        for (int i = 0; i < INetPacket.networkControlFlags.values().length; i++) {
            System.out.println(i + ": " + INetPacket.networkControlFlags.values()[i]);
        }
    }
    ...
    public void printNetworkCommandFrameTypes() {
        ...
        for (int i = 0; i < INetPacket.networkControlFlags.values().length; i++) {
            System.out.println(i + ": " + INetPacket.networkLayerCommands.values()[i]);
        }
    }

    public void initializeRadio() {
        System.out.print("Enter the radio port name (ie: COM3): ");
        String portName = reader.nextLine();
        twoJrRadio = new TwoJRRadioListener(portName);
        twoJrRadio.start();
    }

    public void radioListen() {
        twoJrRadio.listen();
        NetworkPacket netPacket;
        while ((netPacket = twoJrRadio.tryGetNextPacket()) == null) ;

        System.out.println("Network Packet received by TwoJRRadioListener");
        testNetworkPacketAttributes(netPacket);
    }

    public void radioSend(NetworkPacket netPacket) {
        LinkedList<NetworkPacket> netPackets = new LinkedList<>();
        netPackets.push(netPacket);
        if (twoJrRadio.isRadioRunning())
            twoJrRadio.send(netPackets);
        else
            System.err.println("Radio cannot send, not initialized");
    }
    ...
    public JData build(boolean suppressTestAttrPrompt) {
        return build(-1, suppressTestAttrPrompt);
    }

    public JData build(int dataTypeIn) {
        return build(dataTypeIn, false);
    }
    public JData build(int dataTypeIn, boolean suppressTestAttrPrompt) {
            ...
            try {
                ...
                switch (dataType) {
                    case 0:
                        ...
                        JAddress jAddr;
                        String addrInput = reader.nextLine();
                        long addr;

                        // Initialize address
                        if (!addrInput.contains(":")) {
                            addr = Long.parseLong(addrInput);
                            jAddr = new JAddress(addr);
                        } else
                            jAddr = new JAddress(addrInput);
                    ...
                    case 9:
                        System.out.print(buildNetworkPacket().print());
                        return null;
                    case 10:
                        System.out.print(buildApplicationPacket().print());
                        return null;
                    default:
    ...
    NetworkPacket buildNetworkPacket() {
        ...
        ApsPacket apsPacket = buildApplicationPacket();

        NetworkPacket netPacket = new NetworkPacket(seqNumber, networkControl, macAddress, commandFrame, apsPacket);
    ...
    public ApsPacket buildApplicationPacket() {
        return buildApplicationPacket(false);
    }

    public ApsPacket buildApplicationPacket(boolean doPrint) {
        // Application Packet
        boolean invalidData = true;
        while(invalidData) {
            try {
            System.out.println("Application Packet Builder");

            System.out.print("Enter sequence number: ");
            JUnsignedInteger seqNumber = new JUnsignedInteger(JDataSizes.EIGHT_BIT, Integer.parseInt(reader.nextLine()));

            System.out.println("Select command frame: ");
                printApplicationCommandFrameTypes();
                IApsPacket.apsCommands commandFrame = IApsPacket.apsCommands.values()[Integer.parseInt(reader.nextLine())];
            ...
            for(int i=0; i<endPoint.getAttributes().size(); i++) {
                ...
                if(attributeControl.isLengthControl())
                    payload.add((byte) attributeControl.getAttributeMap().get(i).getData().getSize());
                for(byte payloadByte : attributeControl.getAttributeMap().get(i).getData().toByte())
...
*/
            ...
            /*System.out.println("Decoding: ");
            System.out.print(new ApsPacket(apsPacket.toByte()).print());
            */
            //testNetworkPacketAttributes(apsPacket);
            return apsPacket;
            } catch (NumberFormatException ex) {
        ...
        return new ApsPacket(new byte[15]);
    }

    public void printApplicationCommandFrameTypes() {
        int i = 0;
        for(IApsPacket.apsCommands value : IApsPacket.apsCommands.values() ) {
    ...
    public  EndPoint createEndpoint() {
        ...
        while(!validData) {
            System.out.print("Enter the end point name: ");
        String name = reader.nextLine();
            try {
        System.out.print("Enter id: ");
        JInteger jId = new JUnsignedInteger(new byte[] { (byte) Integer.parseInt(reader.nextLine())});

            ArrayList<Attribute> attributes = createAttributes();
            EndPoint endPoint = new EndPoint(name,jId,attributes);
            return endPoint;
    ...
    public Attribute createAttribute(int index) {
        System.out.print("Enter the attribute "+ index + " name: ");
        String name = reader.nextLine();
        System.out.print("Select the attribute data value: ");
        JData data = build(true);

        Attribute attribute = new Attribute(data, name);
        return attribute;
    }
    ...
    public AttributeControl createAttributeControl(EndPoint endPoint) {
        ...
        boolean invalidEntry = true;
        int lengthControlInt = -1;
        /*while (invalidEntry) {
            try {

                System.out.print("Set length Control? (0 - false, 1 true): ");
                lengthControlInt = Integer.parseInt(reader.nextLine());
                if(lengthControlInt > -1 && lengthControlInt < 2)
                    invalidEntry = false;
            }catch(NumberFormatException ex){
                System.out.println("Invalid Entry");
            }
    ...
    public boolean requiresLengthControl(EndPoint endPoint) {
        for (Attribute attribute : endPoint.getAttributes()) {

            if(isDataVariableSize(attribute.getData().getId()))
                return true;
        }
    ...
    public boolean isDataVariableSize(int dataType) {
        switch (dataType) {
            // bit map
    ...
    public void printAttributes(EndPoint endPoint) {
        int i = 0;
        System.out.println("Attributes");
        for(Attribute attribute : endPoint.getAttributes()) {
            String attributeValue = attribute.getName();
            System.out.println((i++) +  ": " + attributeValue);
        }
    }
        ...
        while (run) {
            ...
            switch (command) {
                ...
                case "-dt":
                    ...
                    break;
                case "-ir":
                    demo.initializeRadio();
                    break;
                case "-sd":
                    // Send data over radio
                    //demo.radioSend((NetworkPacket) demo.build(9));
                    break;
                case "-rd":
